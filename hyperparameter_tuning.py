...
env_params = {"parking_angles" : [90, 90],
              "fixed_goal" : [(0, 2), (0, -3), (1, 2), (1, -3)],
        	  "collision_reward": -10,
        	  "reward_p" : 0.5,
...
experiment_params = {"output_dir" : "./results/hyperparameter_tuning_parallel",
...
sweep_configuration = {
        ...
        'num_cells': [256, 512], #[64, 128, 256], # hidden layer size
        'layer_num': [4, 5], #[2, 3, 4], # number of hidden layers
        'max_training_iter': uniform(loc=50_000, scale=250_000),
    }

def main(param_id=24, n_iter=50, folderpath="./results/hyperparameter_tuning_parallel", add_width=0):
    ...
    parser.add_argument('--width', help='increase parking space width', type=float)
    ...
    if args.width is not None:
        env_params["add_width"] = [args.width, args.width]
    
    print(f"start id: {param_id}, n_iter: {n_iter}, output path: {folderpath}, add width: {env_params['add_width']}")
